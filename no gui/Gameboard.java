...
public class Gameboard {
	...
	public Tile updateBoard(String n, int p, Scanner sc) {
		...
		for (int  i = 0; i < COL; i++) {
			for (int j = 0; j < ROW; j++) {
								...
								if (board[i][j].getAnimal().ifOutOfBounds(x, y) == true)
									...
									if (layout.isTrap(x, y) != 0) {
										if (board[x][y].getTerrain() != null) {
											...
											if (layout.isTrap(x, y) != board[i][j].getAnimal().getPNum())
												System.out.println(board[i][j].getAnimal().toString() + " is trapped");
									}
				...
				if (change == 'X') {
					...
					do{
						if (p == 1)
							n = Player.choosePiece('r');
						else
							n = Player.choosePiece('b');
		...
		if (layout.isRiver(a, b)){
			board[a][b] = new Tile(new River(a, b, "~~~~"), a, b);
		}
		if (layout.isTrap(a, b) != 0){
			trapNum = layout.isTrap(a, b);
